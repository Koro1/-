...
(function () {
	window.xareltoDynamicAgendaTemplate = function (config) {
		...
		this.dynamicAgenda = new DynamicAgenda("Glucobay New");
		this.order = 0;
...
xareltoDynamicAgendaTemplate.prototype = {
	...
	sortarray: function (e) {
		...
		if (button.hasClass('active')) {
			for (var i = 0; i < sections.length; i++) {
				if (button.sectioncode != sections[i]) {
					$('#list_' + sections[i]).removeClass("active");
					app.dynamicChapters.removeSection(sections[i])
				}
			}
			xareltoDynamicAgendaTemplate.order = 0;
	...
	playPresentation: function (section) {
		...
		if (res) {
			...
			utils.fastGoTo(sectionMap, 'home');
			//utils.fastGoTo(sectionMap, app.collection.content[app.collection.length - 1]);
			//app.slideshow.scrollToStart();
			//app.collection.scrollToEnd()
			//app.navigationToolbar.goTo(app.collection.content[app.collection.length - 1]);
		}
	},
